...
let line    = `${w.emoji}${ve} \`#${w.serial}` ${re} **${w.variant} ${w.rarity} ${w.name}** | ⚔️ ${weaponAtk(w)} ATK | ${w.element} | ${lvTag}`; if (showXp) line += `\n   📊 ${weaponXpBar(w)} | 🪙 Next: ${weaponUpgradeCost(w)} coins`;
return line;
}
...
`\`${message.author.username}` ${wName}${runeTag}${critTag}  vs  ${mob.emoji} **${mob.name}**\n\n`+`${turnLines}\n\n`+`**Won in ${result.turns} turn(s)!** | 🪙 +${coins} | ✨ +${mob.xp} XP\n`+`❤️ HP: `${hpBar(player.hp, player.maxHp)}``); } else { const hpLoss   = player.hp - Math.max(0, result.hpLeft); const coinLoss = Math.floor(player.coins * 0.1); player.hp      = Math.max(0, result.hpLeft); player.coins  -= coinLoss; save(); return message.reply(`⚔️ **${message.author.username} goes into battle!**\n`+``${message.author.username}` ${wName}${runeTag}  vs  ${mob.emoji} **${mob.name}**\n\n`+`${turnLines}\n\n`+`**Lost in ${result.turns} turn(s)!** | ❤️ -${hpLoss} HP | 🪙 -${coinLoss}\n`+`❤️ HP: `${hpBar(player.hp, player.maxHp)}``
);
}
}
...
`❤️ HP: \`${hpBar(player.hp, player.maxHp)}``); } save(); return message.reply(`💥 Hit **${globalBoss.name}** for **${dmg}** dmg!\n`+`${tierEmoji[globalBoss.tier||“Normal”]} Boss HP: `${bossTierBar(globalBoss.hp, globalBoss.maxHp)}``
);
}
...
if (!tp.inventory[id]) return message.reply(`❌ Weapon \`#${id}` not found in ${target.username}’s inventory.`); const w=tp.inventory[id]; w.level=weaponMaxLevel[w.rarity]||10; w.xp=0; save(); return message.reply(`⚡ Maxed **${w.name}** for **${target.username}**!\n${weaponLabel(w,true)}`);
}
